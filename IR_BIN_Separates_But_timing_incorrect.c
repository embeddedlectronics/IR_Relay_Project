...
void IR_Fun(void){
	...
	delay =  (Pulse_Time[2]+Pulse_Time[3]+Pulse_Time[4]) / 3; 
	cli();  // disable interrupts, no longer needed, reading input
	
	for(char _some_fuckign_counter = 0; _some_fuckign_counter < 98; _some_fuckign_counter++)
	{
	...
	for(char _some_fuckign_counter = 0; _some_fuckign_counter < 7; _some_fuckign_counter++)
	{
		...
		for (char _count_16 =0; _count_16 < 16; _count_16++){
			...
			_stuffs = Store_Pulse[_some_fuckign_counter*16 + _count_16];
				 	
			if (_stuffs) 
				_store_16bits[_some_fuckign_counter] += ( 1 << _count_16); 
		}

	}
	
	for (char _count=0; _count < 7; _count++)
	 printf(" 0x%x \n\r", _store_16bits[_count]); 
